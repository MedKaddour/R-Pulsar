...
/**
 * This is and example of the use of the R-Pulsar API. This example shows how to
 * build a R-Pulsar consumer.
 * 
 * @param keys<
 * @return
 */
public class Consumer {
	...
	public static class Consum implements Runnable {
		...
		Consum(Message.ARMessage msg) {
			this.msg = msg;
			profile = Message.ARMessage.Header.Profile.newBuilder().addSingle("temperature").addSingle("fahrenheit")
					.build();
		...
		public void run() {
			while (running) {
				try {
					...
					consum_msg =GlobalTracing.MigrateContext(msg, consum_msg);
	...
	public static void main(String[] args) throws UnknownHostException, ClassNotFoundException {
		try {
            ...
            TelemetryConfiguration.SERVICE_NAME="Consumer Service";
            ...
            GlobalTracing gT=GlobalTracing.StartSpanAndInjectContext(msg, msg, "Consumer", consumer.getRp());
            span =gT.getSpan();
            msg =gT.getARMessage();
			consumer.post(msg);
